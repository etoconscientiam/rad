...
(function () {
  ...
  function rebuild(s) {
    ...
    if (woodMat) {
      const url = s.p.woodTex;
      if (url) {
        if (s.woodUrl !== url) {
          if (s.woodTex) s.woodTex.dispose();
          const img = new Image(); // без crossOrigin: TextureLoader падает там, где нет CORS-заголовков
          const tex = new THREE.Texture(img);
          img.onload = () => { tex.needsUpdate = true; };
          img.src = url;
          s.woodTex = tex;
          s.woodTex.colorSpace = THREE.SRGBColorSpace;
          s.woodTex.wrapS = s.woodTex.wrapT = THREE.RepeatWrapping;
          s.woodUrl = url;
        }
        const tile = 0.85; // один «лист» текстуры ≈ 850 мм
        s.woodTex.repeat.set(Math.max(0.6, W / tile), Math.max(0.6, L / tile));
        woodMat.map = s.woodTex;
        woodMat.color.set('#FFFFFF');
      } else {
        woodMat.map = null;
        woodMat.color.set(s.p.wood);
      }
    ...
    if (s.p.model === 'deska' || s.p.model === 'pats' || s.p.model === 'chestable') {
      ...
      if (s.p.model === 'chestable') {
        ...
        s.knobMat.color.set(s.p.wood).multiplyScalar(0.62);
  ...
  function Frame3D(props) {
    ...
    s.p = { h: +props.h || 800, w: +props.w || 600, l: +props.l || 1200, profile: +props.profile || 30, color: props.color || '#17171A', model: props.model || 'cube', wood: props.wood || '#C6893B', woodTex: props.woodTex || null, drawerOff: +props.drawerOff || 0, gap: +props.gap || 300 };
    React.useEffect(() => {
      ...
      loadThree().then(THREE => {
        ...
        loop();
        fns.push(() => { cancelAnimationFrame(raf); ro.disconnect(); group.traverse(o => { if (o.geometry) o.geometry.dispose(); }); mat.dispose(); woodMat.dispose(); if (s.woodTex) { s.woodTex.dispose(); s.woodTex = null; s.woodUrl = null; } if (s.knobMat) s.knobMat.dispose(); renderer.dispose(); renderer.domElement.remove(); });
      });
